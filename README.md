...
```bash
flash-sam-lambda$ sam build --use-container
```
...
```bash
flash-sam-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
flash-sam-lambda$ sam local start-api
flash-sam-lambda$ curl http://localhost:3000/
```
...
```bash
flash-sam-lambda$ sam logs -n HelloWorldFunction --stack-name flash-sam-lambda --tail
```
...
```bash
flash-sam-lambda$ pip install -r tests/requirements.txt --user
# unit test
flash-sam-lambda$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
flash-sam-lambda$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name flash-sam-lambda
```
